...
def run_initialization(tag=None, n_layers=None, output_dir=None):
    """
    Run initialization with custom parameters
    
    Args:
        tag: Name tag for this run (default: use global my_tag)
        n_layers: Number of radial layers (default: use global layer)
        output_dir: Output directory (default: use global base_path)
    
    Returns:
        Path to output directory
    """
    global layer, r_list1, r_list2, my_tag, base_path
    ...
    if n_layers is not None:
        layer = n_layers
        # Recalculate radius lists with new layer count
        r_list1 = log_space(r_min, r_max, layer + extra_layer)
        r_list2 = [r_list1[0] / 2] + [(r_list1[i-1] + r_list1[i]) / 2 for i in range(1, len(r_list1))]
